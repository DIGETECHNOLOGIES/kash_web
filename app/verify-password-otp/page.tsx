...
function VerifyPasswordOtpContent() {
    ...
    const handleVerify = async (e: React.FormEvent) => {
        ...
        setError(null);
        try {
            const response = await authApi.verifyOtp(email, verificationCode);

            // This is critical for password reset: we must temporarily login using the token response
            if (response.access) {
                // Ensure auth store accepts it just specifically for password reset purposes
    ...
    const handleResendCode = async () => {
        ...
        try {
            ...
            const err = t('otp.codeResentMsg') || "Code resent to " + email;
            setError(err);
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-background p-4 text-center">
                <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}>
                    ...
                    <h1 className="text-3xl font-black italic tracking-tighter uppercase mb-4">{t('auth.codeVerified')}</h1>
                    <p className="text-text-secondary max-w-sm mx-auto">
                        Redirecting to complete password reset...
                    </p>
                </motion.div>
            </div>
        );
        ...
        <div className="min-h-screen flex items-center justify-center bg-background p-4 relative overflow-hidden">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md z-10">
                <div className="text-center mb-8">
                    ...
                    <h1 className="text-3xl font-black tracking-tight mb-2">{t('auth.enterVerificationCode')}</h1>
