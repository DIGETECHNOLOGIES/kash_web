...
export default function WalletPage() {
    ...
    const { data: transactions, isLoading: isLoadingHistory } = useQuery({
        queryKey: ['wallet-history'],
        queryFn: () => walletApi.getTransactionHistory(),
    });
                    ...
                    <div className="space-y-4">
                        {isLoadingHistory ? (
                            ...
                            transactions?.results?.map((tx: any) => (
                                <Card key={tx.id} className="p-6 rounded-[2rem] flex items-center gap-6 border-border/40 hover:border-primary/30 transition-all shadow-sm">
                                    <div className={`h-14 w-14 rounded-2xl flex items-center justify-center ${tx.type === 'CREDIT' ? 'bg-success/10 text-success' : 'bg-error/10 text-error'}`}>
                                        {tx.type === 'CREDIT' ? <ArrowDownLeft size={24} /> : <ArrowUpRight size={24} />}
                                    </div>
                                    <div className="flex-1">
                                        <h4 className="font-black italic uppercase tracking-tight text-sm mb-1">{tx.description}</h4>
                                        <span className="text-[10px] font-bold text-text-secondary uppercase tracking-widest">{format(new Date(tx.created_at), 'MMM dd, yyyy • HH:mm')}</span>
                                    </div>
                                    <div className={`text-xl font-black italic ${tx.type === 'CREDIT' ? 'text-success' : 'text-error'}`}>
